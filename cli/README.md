...
- resume framework-owner or user-app sessions with `kvdf resume` and `kvdf start`
- enforce framework boundary guardrails with `kvdf guard`
- perform conflict drift scans with `kvdf conflict scan`
- coordinate team GitHub sync with `kvdf sync`
- manage temporary execution queues with `kvdf temp`
- orchestrate multi-AI queues and merge bundles with `kvdf multi-ai`
