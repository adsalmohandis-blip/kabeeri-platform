...
<body class="min-h-screen overflow-x-hidden bg-[#f3e5ab] text-kabeeri-ink antialiased">
    <div class="pointer-events-none fixed inset-0 -z-10 bg-[radial-gradient(circle_at_90%_8%,rgba(17,17,17,.18),transparent_26rem),radial-gradient(circle_at_8%_18%,rgba(17,17,17,.30),transparent_25rem),linear-gradient(135deg,#f3e5ab_0%,#f3e5ab_58%,#111111_100%)]"></div>
    ...
    <div class="mx-auto flex min-h-screen w-[min(1180px,calc(100%-24px))] flex-col px-3 py-4 sm:px-5 lg:py-6">
        <header class="flex flex-col gap-3 rounded-3xl border border-[#111111]/10 bg-[#f3e5ab]/82 p-3 backdrop-blur-2xl lg:flex-row lg:items-center lg:justify-between">
            <a href="{{ route('home') }}" class="flex items-center gap-3">
                <span class="grid h-10 w-10 place-items-center rounded-2xl bg-[#111111] text-sm font-black text-[#f3e5ab]">{{ __('kabeeri.brand.mark') }}</span>
                <span>
            ...
            <nav class="flex flex-wrap gap-2" aria-label="{{ __('kabeeri.ui.public_home') }}">
                ...
                <a class="inline-flex items-center rounded-full bg-[#111111] px-3 py-1.5 text-xs font-black text-[#f3e5ab]" href="{{ route('register') }}"><x-kabeeri-icon name="user-plus" />{{ __('kabeeri.ui.create_account') }}</a>
        ...
        <main class="grid flex-1 gap-5 py-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-center">
            <section class="rounded-[2rem] border border-[#111111]/10 bg-[#f3e5ab]/86 p-5 backdrop-blur sm:p-6 lg:p-7">
                ...
                <div class="mt-5 flex flex-wrap gap-2">
                    <a class="inline-flex items-center rounded-full bg-[#111111] px-4 py-2.5 text-xs font-black text-[#f3e5ab]" href="{{ route('customer.start') }}"><x-kabeeri-icon name="rocket" />{{ __('kabeeri.ui.start_now') }}</a>
            ...
            <aside class="space-y-2.5">
                <article class="rounded-3xl bg-[#111111] p-4 text-[#f3e5ab]">
                    <p class="text-xs font-black tracking-[.08em] text-[#f3e5ab]/70">{{ __('kabeeri.ui.your_steps') }}</p>
                    <ol class="mt-3 divide-y divide-[#f3e5ab]/12 text-sm font-black">
                        <li class="flex items-center gap-2 py-2 first:pt-0"><span class="grid h-6 w-6 place-items-center rounded-full bg-[#f3e5ab] text-[11px] text-[#111111]"><x-kabeeri-icon name="map" style="margin-inline-end:0" /></span><span>{{ __('kabeeri.ui.choose_path') }}</span></li>
                        <li class="flex items-center gap-2 py-2"><span class="grid h-6 w-6 place-items-center rounded-full bg-[#f3e5ab] text-[11px] text-[#111111]"><x-kabeeri-icon name="theme" style="margin-inline-end:0" /></span><span>{{ __('kabeeri.ui.install_app_theme') }}</span></li>
                        <li class="flex items-center gap-2 py-2 last:pb-0"><span class="grid h-6 w-6 place-items-center rounded-full bg-[#f3e5ab] text-[11px] text-[#111111]"><x-kabeeri-icon name="apps" style="margin-inline-end:0" /></span><span>{{ __('kabeeri.ui.open_dashboard') }}</span></li>
                    </ol>
                </article>
            </aside>
        </main>
