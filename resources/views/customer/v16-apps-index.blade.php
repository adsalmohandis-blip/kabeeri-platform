        ...
        <main class="min-w-0 px-4 py-4 sm:px-6 lg:px-8 lg:py-6">
            ...
            <section class="rounded-[1.7rem] border border-[#111111]/10 bg-[#f3e5ab]/92 p-4 shadow-[0_18px_55px_rgba(17,17,17,.08)]">
                <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('customer.apps.create') }}" class="inline-flex items-center justify-center rounded-full bg-[#111111] px-4 py-2 text-xs font-black text-[#f3e5ab]"><x-kabeeri-icon name="plus" />{{ __('kabeeri.ui.new_app') }}</a>
                </div>
                ...
                <div class="divide-y divide-[#111111]/10 overflow-hidden rounded-3xl border border-[#111111]/10 bg-white/70">
                    @forelse ($sites as $site)
                        ...
                        <article class="grid gap-3 px-4 py-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <a class="rounded-full bg-[#111111] px-3 py-2 text-xs font-black text-[#f3e5ab]" href="{{ route('customer.apps.show', ['username' => $site->username]) }}">{{ __('kabeeri.ui.app_detail') }}</a>
                    ...
                    @empty
                        <div class="px-5 py-8 text-center">
                            ...
                            <a href="{{ route('customer.apps.create') }}" class="mt-4 inline-flex rounded-full bg-[#111111] px-4 py-2 text-xs font-black text-[#f3e5ab]">{{ __('kabeeri.ui.create_app') }}</a>
                        </div>
                    @endforelse
                </div>
            </section>
