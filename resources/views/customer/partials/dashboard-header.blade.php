...
<header id="overview" class="relative z-[60] mb-5 rounded-[1.7rem] border border-[#111111]/10 bg-[#f1eadc]/92 px-4 py-3 backdrop-blur-xl">
    <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="inline-flex items-center rounded-full bg-[#111111] px-3 py-2 text-xs font-black text-[#f1eadc]"><x-kabeeri-icon name="logout" />{{ __('kabeeri.ui.logout') }}</button>
            </form>
        </div>
    </div>
</header>
