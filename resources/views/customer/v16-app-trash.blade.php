                    ...
                    @forelse ($trashedSites as $site)
                        <article class="grid gap-3 px-4 py-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                            <div>
                                ...
                                <span class="mt-1 block text-xs font-bold text-[#000000]/60">{{ __('kabeeri.ui.scheduled_delete') }}: {{ $site->metadata['trash_scheduled_delete_at'] ?? '-' }}</span>
                            </div>
                            <div class="flex flex-wrap gap-2">
                                <form method="POST" action="{{ route('customer.apps.restore', ['username' => $site->username]) }}">
                                    @csrf
                                    <button class="rounded-full bg-[#000000] px-3 py-2 text-xs font-black text-[#f0f0f0]" type="submit">{{ __('kabeeri.ui.restore') }}</button>
                                </form>
                                <form method="POST" action="{{ route('customer.apps.schedule-delete', ['username' => $site->username]) }}" class="flex gap-2">
                                    @csrf
                                    @method('PATCH')
                                    <select name="retention_days" class="rounded-full border border-[#000000]/10 bg-[#f0f0f0] px-3 py-2 text-xs font-black">
                                        <option value="30">30</option>
                                        <option value="60">60</option>
                                        <option value="90">90</option>
                                    </select>
                                    <button class="rounded-full bg-[#fafafa] px-3 py-2 text-xs font-black text-[#000000] ring-1 ring-[#000000]/10" type="submit">{{ __('kabeeri.ui.schedule_delete') }}</button>
                                </form>
                            </div>
                        </article>
                    @empty
